package ClassesConstructorsInheritance;
/* Write a class with the name Circle.
The class needs one field with name radius of type double.
The class needs to have one constructor with parameter radius of type double and it needs to initialize the fields.
In case the radius parameter is less than 0 it needs to set the radius field value to 0.
Write the following methods:
1. getRadius without any parameters.
2. getArea without any parameters, it needs to return the calculated area.

 */

public class Circle {
    public static void main(String[] args) {
        Circle circle = new Circle(3.75);
        System.out.println("circle.radius = "+circle.getArea());
        System.out.println("circle.area = "+circle.getArea());
    }

    private double radius;

    public Circle(double radius) {
        if (radius < 0) {
            radius = 0;
        }
        this.radius = radius;
    }

    public double getRadius() {
        return radius;
    }
     public double getArea(){
         return radius*radius*Math.PI;
     }

}

package Composition;
/* Encapsulation
Create class and name it Printer.
This class will have two member variables of type int, tonerLevel and pagesPrinted, one one of type boolean called duplex.
...
If does not meet these conditions then it should be initialized to -1.
...
1. addToner will accept one parameter, tonerAmount of type int. First off, tonerAmount should be greater than 0 and less thar or equal to 100.
If this condition is met a second test must be included to test whether tonerLevel plus tonerAmount is greater than 100.
If so, the method should return -1. If not then tonerLevel should have the incoming tonerAmount added to it and the nev tonerLevel should be returned by the method.
...
The pagesToPrint value is then added to the class field pagesPrinted, but the value of pagesToPrint should returned by the method.
